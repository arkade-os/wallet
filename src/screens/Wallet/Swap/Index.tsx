...
export default function WalletSwap() {
  ...
  const assets = useMemo(
    () => toSwapAssets(rows, config.currency, fiatDecimals(), fromFiatAmount, toFiat, toFiatAmount),
    [rows, config.currency, fiatDecimals, fromFiatAmount, toFiat, toFiatAmount],
  )
...
function toSwapAssets(
  ...
  const mapped = rows.map((row) => ({
    ...
    usdPrice: estimateRowUsdPrice(row, fiat, fromFiatAmount, toFiat, toFiatAmount),
  }))
