...
export default function SendForm() {
  ...
  useEffect(() => {
    ...
    const parseRecipient = async () => {
      ...
      if (!recipientFocusRef.current) setRecipientValidationError('Invalid recipient address')
    }

    // paste, scan, blur, Enter and clear parse right away; typing debounces
    if (!recipient || immediateParseRef.current) {
      parseRecipient()
      return
    }

    const timer = setTimeout(parseRecipient, 400)
  ...
  const handleSelectAsset = (asset: AssetOption | null) => {
    ...
    immediateParseRef.current = true // re-validate the recipient right away
    if (asset) {
      if (isBTCAddress(recipient)) {
        return setRecipientValidationError('Assets can only be sent to Arkade addresses')
      }
