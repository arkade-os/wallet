...
const fetcher = <T>(p: Promise<Response>): Promise<T> => p.then((r) => (r.ok ? (r.json() as T) : Promise.reject(r)))
...
export class EscrowClient {
  ...
  async signin(priv: Uint8Array<ArrayBufferLike>) {
    ...
    const challengeRes = await fetcher<{ hashToSignHex: string; challengeId: string }>(
      fetch(`${this.baseUrl}/api/v1/auth/signup/challenge`, {
        method: 'POST',
        headers: {
    ...
    const signatureHex = schnorr.sign(hexToBytes(challengeRes.hashToSignHex), priv)

    const { accessToken } = await fetcher<{ accessToken: string }>(
      fetch(`${this.baseUrl}/api/v1/auth/signup/verify`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Origin: 'http://localhost:test',
