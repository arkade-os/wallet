...
// The payer path lives in @arkade-os/lnurl-client. What remains here is
// fetchArkAddress, which calls the callback with ?method=ark — a parameter the
// Arkade lnurl-server does not implement, so the package has no equivalent by
// design. Removing it would change behaviour for whichever provider it does
// serve, so it stays until then.
