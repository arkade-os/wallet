 ...
 * an indexer call and a localStorage write issued from whatever screen happened
 * to be open. `RfqSwapManager` owns that answer now and records it as the swap
 * ends (see `providers/lnSwaps`); the history carries it onto the row, and this
 * only reads. A row still carrying the legacy `lnSend` metadata — a send made
 * before the records existed — is read from there instead, so old receipts keep
...
export function useLnSendReceipt(tx: Tx | undefined): LnSendReceipt | undefined {
  ...
  const refunded = swap ? swap.state === 'refunded' : legacy?.spend?.outcome === 'refunded'
