...
export const AssetSwapsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const { dataReady, reloadWallet, txs } = useContext(WalletContext)
  // Per CAPACITOR.plan.md § Wallet Exposure Decision, only provider-level
  // adapters get the raw wallet — offer creation/cancellation need it directly.
  const svcWallet = useWalletRuntime()?.wallet
