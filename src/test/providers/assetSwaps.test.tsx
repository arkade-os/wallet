...
// AssetSwapsProvider reads the raw wallet from useWalletRuntime (per CAPACITOR.plan.md
// § Wallet Exposure Decision), not WalletContext — stub the hook rather than the context.
const walletRuntimeRef = vi.hoisted(() => ({ current: undefined as { wallet: any } | undefined }))

vi.mock('../../providers/wallet', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../../providers/wallet')>()),
  useWalletRuntime: () => walletRuntimeRef.current,
}))
...
function renderProvider(reloadWallet = vi.fn().mockResolvedValue(undefined)) {
  walletRuntimeRef.current = { wallet: { identity: {} } }
  render(
    <AspContext.Provider
      value={{ ...mockAspContextValue, aspInfo: { ...mockAspContextValue.aspInfo, network: '', url: '' } } as any}
    >
      <WalletContext.Provider value={{ ...mockWalletContextValue, reloadWallet } as any}>
        <AssetSwapsProvider>
          <CancelHarness />
        </AssetSwapsProvider>
...
function renderCreateProvider(plan: OfferPlan) {
  ...
  walletRuntimeRef.current = { wallet: { identity: {}, send } }
  render(
