...
// swap history. Steps:
// 1. Create new wallet
// 2. Perform a reverse swap (Lightning → Arkade)
// 3. Perform a submarine swap (Arkade → Lightning)
// 4. Reset wallet
// 5. Restore wallet with backup phrase (nsec)
// 6. Verify swap history has both swaps
...
test('should restore swaps without nostr backup', async ({ page, isMobile }) => {
  ...
  // verify both swaps are present (swap recovery from Boltz API can take a moment)
