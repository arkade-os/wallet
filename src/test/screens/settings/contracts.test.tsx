...
// Partial WalletContext override exposing the given contracts through the
// runtime-neutral `advanced` capability group. `undefined` → wallet not ready.
function withContracts(contracts: Contract[]) {
  return { walletReady: true, advanced: advancedWithContracts(contracts) }
}

function renderScreen(walletOverride: ReturnType<typeof withContracts> | undefined, aspInfo = emptyAspInfo) {
  return render(
    <AspContext.Provider value={{ aspInfo, setAspInfo: () => {} } as any}>
      <WalletContext.Provider
        value={{ ...mockWalletContextValue, ...(walletOverride ?? { walletReady: false }) } as any}
      >
        <Contracts />
      </WalletContext.Provider>
    </AspContext.Provider>,
