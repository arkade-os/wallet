...
/** A row as `graftLnSends` builds it from a swap record. */
const lnSwapTx = (over: Partial<NonNullable<Tx['lnSwap']>>): Tx => ({
  ...sentTx,
  lnSwap: { state: 'pending', fundingTxid, ...over },
})
...
describe('Lightning send receipt', () => {
  ...
  it('shows both legs once the solver has claimed', async () => {
    renderReceipt(lnSwapTx({ state: 'settled', spendTxid: 'claim-txid' }))
  ...
  it('calls the second leg a refund, not a cancellation, when the funds came back', async () => {
    ...
    renderReceipt(lnSwapTx({ state: 'refunded', spendTxid: 'refund-txid' }))
