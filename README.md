...
# regenerate the app icon from resources/ (cap add resets it to the Capacitor default)
pnpm run cap:icons
...
The native app icon is generated by `pnpm run cap:icons` (see `scripts/generate-capacitor-icons.mjs`)
from the source images in `resources/` (`icon-only.png`, `icon-background.png`, `icon-foreground.png`).
Since `android/` and `ios/` are regenerated scaffold, re-run it after `npx cap add` recreates a platform.
